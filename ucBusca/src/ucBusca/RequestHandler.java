...
public class RequestHandler extends Thread{
	...
	public String loginHandler(HashMap<String, String> data){
		...
		if(client_is_registered==true && client_is_logged==false){
			...
			if(aux==true){
				...
				res="request_ID | " + data.get("request_id") + " ; username | " + username + " ; type | status ; logged | on ; msg | Logged!";
			}
			else
	...
	public void run(){
		...
		try{			
			...
			else if(message.contains("offline") || message.contains("registry") || message.contains("login") || message.contains("registers_list") || message.contains("admin_privileges") || message.contains("index_url") || message.contains("search") || message.contains("my_searches")){								
